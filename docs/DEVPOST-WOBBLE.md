...
## What's next
Tower themes beyond the default Leaning-Tower-of-Pisa look, real Reddit flair ranks for master builders, per-subreddit tower leaderboards, and shareable memorial cards.
```
...
Reddit Developer Platform - **Devvit Web** (`@devvit/web@0.13.7`) + Express + Redis. The tower's tilt is a **single deterministic number** (no physics engine), so every client agrees on state and culprit; real physics is used only for the collapse animation. Turns use an **atomic Redis SET‑NX lock** (token‑verified, no double‑commit). The server **clamps every drop** - one rule that guarantees both anti‑cheat and the anti‑grief invariant (a fresh tower survives any single bad storey - unit‑tested). Memorials are posted by the app account. **No AI.** 16 unit tests on the math + core. Tap‑only, sub‑second, mobile‑first webview.
